...
| 로그인 시도 제한 | 인메모리 제한 적용, 단일 인스턴스 배포만 지원 | 멀티 인스턴스 전환 시 공유 상태 부재 |
| 이벤트 수집 과다 요청 | `/api/events`(60/min), `/api/web-events`(120/min), `/api/public-download`(기본 10/min) 제한, 단일 인스턴스 배포만 지원 | 분산 환경에서 전역 limit 부재 |
...
- 위험: 공개 다운로드 파일/메타 변조.
  ...
  - 완화: 업로드 시 SHA-256 해시를 current.json에 기록(RISK-014 완료). 운영 점검 절차는 docs/maintenance.md 참조.

### R: 부인(Repudiation)
- 위험: 관리자 설정 변경/내보내기 행위 추적 부족.
  - 현재: 업로드와 raw query export에 대해 요청자/조건/결과를 포함한 구조화 감사 로그 기록.
  - 공백: 영속 감사 로그 저장소는 없고 애플리케이션 로그 보존 정책에 의존.
  - 완화: 운영 로그 보존 기간 관리, 필요 시 별도 감사 저장소 검토.
...
- 위험: 웹 방문 이벤트 확장(referrer/utm/query/context) 수집으로 로그 내 과수집/민감정보 유입 가능성.
  - 현재: referrer URL은 query/fragment 제거 후 저장, 도메인/UTM/기기 분류 중심 집계.
  - 완화: 허용 경로 최소화(RISK-017 완료 — 최대 20개 상한, `"/"`사용 시 시작 경고, 운영 가이드 최소 경로 원칙 명시), 길이 제한/null 정규화, 정기 보존/파기 점검.

### D: 서비스 거부(Denial of Service)
- 위험: 대용량 업로드/이벤트 과다 입력.
  - 현재: 업로드 용량/행 제한 + 이벤트 수집 API 레이트리밋 적용.
  - 공백: 인메모리 구현으로 멀티 인스턴스 전역 제한은 제공하지 않음.
  - 완화: 현행 운영은 단일 인스턴스로 제한하고, 수평 확장 전 공유 스토어 기반 글로벌 레이트리밋을 선행 도입.
- 위험: 반복 로그인 시도.
  - 현재: 인메모리 레이트 리미터.
  - 공백: 다중 인스턴스에서 무력화 가능.
  - 완화: 현행 운영은 단일 인스턴스로 제한하고, 멀티 인스턴스 요구 발생 시 공유 스토어 기반 리미터로 확장.
...
- 인메모리 레이트 리미팅은 수평 확장 환경에서 약하므로 현재는 단일 인스턴스 운영 정책으로 통제한다.
- 원시 로그 내보내기 데이터의 2차 유출 위험은 운영 통제에 의존.
- 공개 다운로드 파일 배포 프로세스의 무결성 검증 체계: SHA-256 해시 기록으로 기본 체계 확립(RISK-014 완료).

## 권장 후속 조치
1. 첫 수평 확장 요구 시 Redis/Valkey 등 공유 스토어 기반 로그인/이벤트 limiter 도입.
2. 관리자 액션(업로드, 설정변경, 내보내기) 감사 로그 강화.
3. 공개 다운로드 파일 무결성 체크(해시/서명 또는 점검 절차) 도입 — SHA-256 해시 기록 완료(RISK-014).
